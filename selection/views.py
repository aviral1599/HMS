...
from selection.models import RoomClean, Student, Room, Hostel,Course
from django.contrib.auth import authenticate, login, logout
from django.contrib.auth.decorators import login_required
from .forms import FeedbackForm,ComplaintsForm,RoomCleanForm
...
@login_required
def layout(request,hostel_name):
    # rooms = ["A111", "A112", "A113", "A114", "A115"]
    this_hostel = Hostel.objects.get(name=hostel_name)
    rooms = Room.objects.filter(hostel=this_hostel)
...
def select(request):
    ...
    if request.method == 'POST':
        ...
        # hostel = Hostel.objects.filter(gender=student_gender, course=student_course)
        x = Room.objects.filter(room_type=['S', 'D'], vacant=True)
...
@login_required
def roomclean_form(request):
    if request.method == 'POST':
        form = RoomCleanForm(request.POST)
        ...
        form = RoomCleanForm()
    return render(request, 'room_clean.html', {'form': form})
